...
import type { ShippingRatesResponse } from "@craftynp/types";
...
function isShippingRatePayload(value: unknown): value is ShippingRatePayload {
...
export async function POST(request: NextRequest) {
  ...
  if (!isShippingRatePayload(body)) {
    return NextResponse.json({ error: "invalid_body" }, { status: 400 });
  }

  try {
    const response = await sdk.client.fetch<ShippingRatesResponse>(
      "/store/shipping-rates",
      { method: "POST", body },
    );
    return NextResponse.json(response);
  } catch (error) {
    console.error("Could not load shipping rates", error);
