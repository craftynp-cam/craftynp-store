...
import { readCartCount, subscribeToCartCount } from "@/lib/cart-count";

import { ShoppingCartSimple } from "../icons";

/**
 * The count is genuinely live (CNP-24 AC 2): it reads `cart-count.ts` through
 * `useSyncExternalStore`, the same pattern `ThemeToggle` uses for the theme
 * preference. The server cannot know the count, so it renders empty and the
 * client corrects it on hydration — that's also the correct empty state, so
